...
// what the page does, via the same web/crypto.js module (decode code → KDF →
// claim → decrypt → parse envelope). Text payloads go to stdout as text; file
// payloads go to stdout as raw bytes with the header JSON on stderr.
// Usage: node tests/e2e.mjs '<url>#<code>' '<PIN>' [--expect-status 410]
import { decodeCode, derive, decrypt, parseEnvelope, toHex } from '../web/crypto.js';
...
const u = new URL(url);
let locator, key;
try { ({ locator, key } = decodeCode(decodeURIComponent(u.hash.slice(1)))); } catch (e) { console.error(`bad code: ${e.message}`); process.exit(2); }

const keys = await derive(key, pin, locator);
const resp = await fetch(`${u.origin}/api/secrets/${locator}/claim`, {
  method: 'POST',
...
const plain = await decrypt(keys.enc, locator, new Uint8Array(Buffer.from(body.ct, 'base64')));
